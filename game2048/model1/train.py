...
if not os.path.exists(args.save_dir):
    ...
    print('save_dir ',args.save_dir,' creates')
else:
    print('save_dir ',args.save_dir,' exists')

if len(args.data_dir) == 0:
    args.data_dir = 'data_dir'
if not os.path.exists(args.data_dir):
    os.mkdir(args.data_dir)
    print('data_dir ',args.data_dir,' creates')
else:
    print('data_dir ',args.data_dir,' exists')

if len(args.test_dir) == 0:
    args.test_dir = 'test_dir'
if not os.path.exists(args.test_dir):
    os.mkdir(args.test_dir)
    print('test_dir ',args.test_dir,' creates')
else:
    print('test_dir ',args.test_dir,' exists')
...
if __name__ == '__main__':
    if args.device > -1:
        ...
        if args.scratchtrain == 0: #pre-train continue
    ...
    for epoch in range(args.num_epoch):
        ...
        train_report = "\n train [%d/%d] Loss: %7f, Acc: %.3f"%(epoch+1,args.num_epoch,running_loss,100*running_acc)
        print(train_report)
        with open(os.path.join(args.save_dir,'train_log.txt'),'a') as f:
